...
function getWeather(lat, longti)
{
	var xhr = new XMLHttpRequest();
    var url='https://api.openweathermap.org/data/2.5/weather';
    var queryparam = '?appid=37ea973b5e1efc1e297585e3f1440598';
    queryparam += '&units=metric';
    queryparam += '&lat='+lat;
    queryparam += '&lon='+longti;
    url+=queryparam;
    var json;
    var msg;
    var weather;
    xhr.onreadystatechange = function()
    {
        if (xhr.readyState === xhr.DONE)
        {
            if (xhr.status === 200 || xhr.status === 201)
            {
            	...
            	cod = json.weather[0].id;
            	weather = wDescEngToKor(cod);
            	msg = '현재 온도 : '+json.main["temp"]+'도<br>습도'+json.main.humidity+' %<br>현재 날씨 : ' + weather;
            	msg += '<img src=http://openweathermap.org/img/w/' + json.weather[0].icon + '.png>';
            	document.getElementById("weatherDiv").innerHTML = msg;
            }
        }
    };
    xhr.open('GET', url);
